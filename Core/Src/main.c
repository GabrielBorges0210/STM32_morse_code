...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == TIM3) 
    {
        ...
        {
            silence_timer++;
            if (silence_timer == 240) pending_space = 1;
            if (silence_timer == 560) { pending_space = 0; pending_word = 1; }
            if (silence_timer == 1000) flag_silencio_linha = 1; 
        }
        ...
        if (tx_state != TX_IDLE) 
        {
            ...
            if (tx_state == TX_PONTO || tx_state == TX_TRACO) {
                if (tx_timer % 2 != 0) TIM2->CCER |= TIM_CCER_CC2E;  
                else TIM2->CCER &= ~TIM_CCER_CC2E; 
            } else {
                TIM2->CCER &= ~TIM_CCER_CC2E; 
            }
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART2) 
    {
        ...
        else { tx_read_idx = 0; tx_write_idx = 0; }
